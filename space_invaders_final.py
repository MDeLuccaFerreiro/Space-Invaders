...
def mostrar_ranking():
    ...
    titulo = fuente_grande.render("Ranking", True, (255, 255, 255))
    screen.blit(titulo, (320, 30))

    if not ranking:
        mensaje = fuente_mediana.render("No hay puntajes registrados aún.", True, (200, 200, 200))
        screen.blit(mensaje, (250, 120))
    else:
        for i, entrada in enumerate(ranking):
            texto_ranking = fuente_mediana.render(f"{i + 1}. {entrada['nombre']} - {entrada['puntaje']}", True, (200, 200, 200))
            screen.blit(texto_ranking, (250, 120 + i * 40))

    texto_volver = fuente_mediana.render("Presiona ESC para volver", True, (200, 200, 200))
    screen.blit(texto_volver, (250, 400))
    pygame.display.update()

    # Esperamos que el jugador presione ESC para volver al menú
    esperando = True
    while esperando:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    ...
                    if seleccion == "Iniciar Juego":
                        nombre_jugador = pedir_nombre()  # Pedimos el nombre aquí
                        iniciar_juego(False, nombre_jugador)  # Llamamos a iniciar el juego con el nombre
                    elif seleccion == "Tutorial":
                        mostrar_tutorial()
                    elif seleccion == "Multijugador":
                        nombre_jugador = pedir_nombre()  # Pedimos el nombre para el multijugador
                        iniciar_juego(True, nombre_jugador)  # Iniciamos el juego con el nombre
                    elif seleccion == "Ranking":
                        mostrar_ranking()  # Mostramos el ranking
